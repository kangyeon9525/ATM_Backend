    ...
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http //인증되지 않은 모든 페이지의 요청을 허락
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/user/signup", "/user/login").anonymous() //익명의 사용자만 login, signup 접근 가능
                        .requestMatchers("/user/delete").authenticated() //인증된 사용자만 delete 접근 가능
                        .anyRequest().permitAll()) //그 외 모든 요청은 모든 사용자가 접근 가능
